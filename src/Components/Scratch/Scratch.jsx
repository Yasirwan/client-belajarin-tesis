        ...
        <div class="flex items-center justify-center flex-col">
          {userType == "Admin"? (
            ...
            <button
              ...
              onClick={() => deleteScratchFunc(data._id)}> <FiTrash2 className="text-red-500 mr-2"/> Hapus</button>
            </>
          ) : (
            <button className="deleteScratch bg-white shadow-lg w-full rounded-full px-4 py-2 flex items-center justify-center cursor-pointer text-black hover:transition hover:duration-150 hover:transform hover:translate-y-1 mb-2" onClick={() => handleClick(data._id)}> <IoPlayOutline className="text-green-500 mr-2"/>Mulai</button>
          )}
